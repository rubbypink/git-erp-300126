const NotificationPanelRenderer = (function () {
    ...
    const SELECTORS = {
        ...
        reloadBtn: '#reloadNotifs', // <--- Nút làm mới
        loadMoreBtn: '#loadMoreNotifs', // <--- Nút xem thêm
    ...
    function render(notifications, unreadCount) {
        ...
        const count = unreadCount != null ? unreadCount : 0;
        ...
        items.forEach((notif) => {
            // ★ BỘ LỌC CHỐNG DATA-CHANGE (Lớp bảo vệ cuối cùng)
            if (notif.type === 'data-change') return;
    ...
    function _createNotificationItem(notif) {
        ...
        item.innerHTML = `
            <div class="d-flex align-items-start p-2">
                ...
                <div class="notification-content flex-grow-1">
                    <div class="fw-bold " style="font-size: 0.9rem;">${_escapeHtml(notif.title)}</div>
        ...
        item.addEventListener('click', async (e) => {
            if (!notif.isRead && window.NotificationManager) {
                await window.NotificationManager.markAsRead(notif.id);
    ...
    function _formatTime(iso) {
        ...
        const isSameDay = now.getFullYear() === notifDate.getFullYear() && now.getMonth() === notifDate.getMonth() && now.getDate() === notifDate.getDate();
