...
function handleGlobalShortcuts(e) {
  ...
  for (const [funcName, assignedCombo] of Object.entries(APP_SHORTCUTS)) {
    if (assignedCombo === combo) {
      ...
      log(`🔥 Executing Shortcut: ${combo} -> ${funcName}`);
...
function loadShortcutsToForm() {
  ...
  Object.entries(APP_SHORTCUTS).forEach(([funcCall, keyCombo]) => {
    ...
    newItem.className =
      'list-group-item d-flex justify-content-between align-items-center py-3 px-0';
    newItem.id = scId;
    newItem.innerHTML = `
            <div>
...
function saveNewShortcut() {
  ...
  if (typeof window[funcName] !== 'function') {
    const confirm_add = confirm(
      `⚠️ Hàm "${funcName}" không tìm thấy trong hệ thống. Bạn có muốn tiếp tục thêm không?`
    );
