...
function toggleContextUI(targetTabIdOrIndex) {
  try {
    ...
    if (activeTabIndex === TAB_INDEX_BY_ID['tab-form']) {
      ...
      A.Event?.trigger('btn-select-datalist', 'change');
...
function renderSecondaryIndexTable(key, tblId) {
  ...
  generateGridColsFromObject(A.DB.schema[key]?.source ?? key);
...
function renderSecondaryIndexFromFlat(key, flatData, tblId) {
  ...
  try {
    const schemaDef = A.DB.schema[key],
      schemaKey = schemaDef?.source ?? key,
      groupBy = schemaDef?.groupBy ?? 'id';
    if (!flatData?.length) {
      if (tbody) tbody.innerHTML = `<tr><td colspan="100%" class="text-center p-4 text-muted">Không có dữ liệu (${key})</td></tr>`;
      return;
    }
    if (!GRID_COLS?.length) generateGridColsFromObject(schemaKey);
    if (!GRID_COLS?.length) {
      if (tbody) tbody.innerHTML = `<tr><td colspan="100%" class="text-center p-4 text-muted">Không tìm thấy cấu hình cột</td></tr>`;
      return;
    }
    if (thead) thead.innerHTML = '<th style="width:50px" class="text-center">#</th>' + GRID_COLS.map((col) => `<th class="${col.hidden ? 'd-none ' : ''}text-center" data-field="${col.key}">${col.t}</th>`).join('');
    const grouped = {};
    ...
    Object.entries(grouped).forEach(([groupValue, docs]) => {
      ...
      groupTr.innerHTML = `<td colspan="${GRID_COLS.length + 1}" class="ps-3 py-1 fw-bold"><span class="badge bg-primary me-2">${docs.length}</span><span class="text-muted small fw-normal me-1">${groupByLabel}:</span><span>${groupValue}</span></td>`;
      ...
      docs.forEach((row, idx) => {
        ...
        html += GRID_COLS.map((col) => {
          ...
          if (col.fmt === 'money' && typeof formatNumber === 'function') val = formatNumber(val);
          if (col.fmt === 'date' && typeof formatDateVN === 'function') val = formatDateVN(val);
          return `<td class="${col.align}${col.hidden ? ' d-none' : ''}">${val}</td>`;
        }).join('');
        tr.innerHTML = html;
        tr.style.cursor = 'pointer';
        tr.id = row.id || `${key}-${idx}`;
        tr.dataset.item = row.id || `${key}-${idx}`;
        tr.onmouseover = function () {
          this.classList.add('table-active');
        };
        tr.onmouseout = function () {
...
function getAppDataFlat(key) {
  ...
  if (A.DB?.schema?.[key]?.isSecondaryIndex) return Object.values(data).flatMap((group) => (group && typeof group === 'object' ? Object.values(group) : []));
...
function initFilterUI() {
  ...
  if (GRID_COLS?.length > 0) {
    ...
    if (GRID_COLS.length > 1 && select.selectedIndex === -1) select.selectedIndex = 0;
  } else {
    select.innerHTML = '<option value="-1">...</option>';
  }
  A.Event.on('filter-col', 'change', updateFilterDataList);
  updateFilterDataList();
}

function updateFilterDataList() {
  const rawCol = String(getE('filter-col')?.value ?? '').trim(),
    datalist = getE('filter-datalist');
  if (!datalist || rawCol === '-1' || rawCol === '') return;
  datalist.innerHTML = '';
  const sourceData = getAppDataFlat(GRID_STATE.currentTable);
  if (!sourceData.length) return;
  const distinctValues = new Set(),
    colCfg = GRID_COLS.find((c) => String(c.i) === rawCol || String(c.key) === rawCol);
  const fieldName = colCfg?.key || colCfg?.i || rawCol;
  sourceData.forEach((row) => {
    let val = typeof row === 'object' && !Array.isArray(row) ? row[fieldName] : row[rawCol];
    if (val) distinctValues.add(String(val).trim());
  });
  setDataList('filter-datalist', [...distinctValues].sort((a, b) => b.localeCompare(a)).slice(0, window.A?.getConfig?.('filter_datalist_limit') ?? 100));
  setVal('filter-val', '');
}
...
function initBtnSelectDataList(data) {
  ...
  if (!hasOption) {
    ...
    if (data['bookings']) {
      ...
      A.Event.trigger(selectElem, 'change');
    }
  }
}
...
function updateDashHeaderStats(tableName, tableId, count = 0, total = 0) {
  ...
  const configs = {
    ...
    BK_CHO_CODE: { title: `Booking Chờ Code (${dToCalc > badgeDays * 5 ? dToCalc : badgeDays * 5} Ngày)`, badgeClass: 'pending' },
    BK_CHO_THANH_TOAN: { title: `Booking Cần TT (${dToCalc > badgeDays * 5 ? dToCalc : badgeDays * 5} Ngày)`, badgeClass: 'pending' },
    // Config cho role OP
...
function renderDashTable_MissingSupplier_Op() {
  ...
  const entries = Object.values(APP_DATA.operator_entries || {})
    .filter((r) => (!r.supplier || !r.supplier.trim()) && new Date(r.check_in) >= today)
    .sort((a, b) => new Date(a.check_in) - new Date(b.check_in));
  const cols = [
    { key: 'id', label: 'SID' },
...
function renderDashTable_NewPrices_Op() {
  ...
  allPrices.forEach((row) => {
    ...
    tr.ondblclick = async () => {
      if (row.type === 'Khách Sạn') await A.HotelPriceController?.init('dynamic-modal-full-body');
