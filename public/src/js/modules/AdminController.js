...
window.o = async function (fn) {
    if (!fn) fn = getVal('global-search');
    let res;
    if (typeof fn === 'function') res = await fn();
    else res = await runFn(fn);
    if (!res) return logA('❌ Lỗi: Function not found.', 'error');
    A.Modal.render(res, 'Search');
    A.Modal.show();
};
