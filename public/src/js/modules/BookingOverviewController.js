...
const BookingOverviewController = (function () {
    ...
    async function open(bookingId, options = {}) {
        ...
        try {
            ...
            await A.UI.renderModal('tpl_booking_overview.html', `Booking - ${bookingId}`, null, null, {
                footer: 'false', // Nếu false thì không hiển thị footer
                size: 'modal-xl',
            });

            if (!_modalRef) A.Modal.show();
            else _modalRef.show();

            await new Promise((resolve) => setTimeout(resolve, 200));
            _rootEl = _modalRef?._getEl() || getE('bkov-root');
    ...
    function _renderTransactions() {
        ...
        <tr>
          ...
          <td><span class="badge bg-secondary bg-opacity-10  border">${t.status || 'Completed'}</span></td>
        </tr>`;
            })
            .join('');
    }
    ...
    async function _saveTab(tabKey) {
        try {
            ...
            switch (tabKey) {
                case 'detail':
                    result = await _saveBookingInfo();
                    break;
                case 'customer':
                    result = await _saveCustomerInfo();
                    break;
                case 'services':
                    result = await _saveServicesInfo();
                    break;
                case 'notes':
                    result = await _saveBookingInfo();
                    break;
    ...
    function _addInternalNote() {
        ...
        if (_bookingId && A.DB) {
            A.DB.arrayUnionField('bookings', _bookingId, 'note_internal', entry)
                .then(() => {
                    if (_bookingData) {
