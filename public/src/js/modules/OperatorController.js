...
export const SupplierPayment = {
  ...
  async openFilterDialog() {
    try {
      ...
      const suppliers = APP_DATA?.lists?.supplier;
      let supplierOptions = '<option value="">-- Chọn Nhà Cung Cấp --</option>';
      suppliers.forEach((s) => {
        supplierOptions += `<option value="${s}">${s}</option>`;
      });
      ...
      const { value: filterParams } = await Swal.fire({
        ...
        didOpen: () => {
          ...
          document.getElementById('swal-date-preset').addEventListener('change', function (e) {
            if (typeof getDateRange === 'function') {
              const range = getDateRange(e.target.value);
              if (range && range.start && range.end) {
                document.getElementById('swal-start-date').value = formatDateForInput(range.start);
                document.getElementById('swal-end-date').value = formatDateForInput(range.end);
              }
            }
          });
        },
  ...
  async processData({ supplier, start, end }) {
    try {
      ...
      let filtered = HD.filter(sourceData, supplier, '==', 'supplier');

      // Lọc tiếp theo Date Range (check_in)
      if (start && end) {
        filtered = Object.values(filtered).filter((item) => item.check_in >= start && item.check_in <= end);
      }
      ...
      filtered.forEach((rowData) => {
        if (typeof addDetailRow === 'function') addDetailRow(rowData);
      });
  ...
  _setupActionContainer(cost, paid, debt, typeHtml) {
    ...
    const oldContainer = document.getElementById('sup-action-container');
    ...
    const tableArea = document.getElementById('detail-tbody').closest('.card');
    ...
    try {
      ...
      this.state.entries.forEach((entry) => {
        if (entry.dept_balance > 0) {
          const entryRef = db.collection('operator_entries').doc(entry.id);
   ...
   */
  closeView() {
    const container = document.getElementById('sup-action-container');
