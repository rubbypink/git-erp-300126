    ...
    function loadBookingToUI(bkData, customerData, detailsData) {
      if (!bkData) return;
      try {
    ...
    function addDetailRow(data = null) {
      ...
      tr.innerHTML = `
        ...
        <td>
          <select class="form-select form-select-sm d-type" data-field="service_type" onchange="Sales.onTypeChange(${idx})">
            <option value="">-</option>${optsType}
          </select>
        </td>
        <td>
          <select class="form-select form-select-sm d-loc" data-field="hotel_name" onchange="Sales.onLocationChange(${idx})">
            <option value="">-</option>
          </select>
        </td>
        <td>
        ...
        <td><input type="date" class="form-control form-control-sm d-in" data-field="check_in" onchange="Sales.autoSetOrCalcDate(this.value, $('.d-out', $('#row-${idx}')))" style="cursor:pointer"></td>
        <td><input type="date" class="form-control form-control-sm d-out" data-field="check_out" onchange="Sales.calcRow(${idx})"></td>
        ...
        <td><input type="number" class="form-control form-control-sm d-qty number" data-field="quantity" value="1" onchange="Sales.calcRow(${idx})"></td>
        <td><input type="number" class="form-control form-control-sm d-pri number" data-field="unit_price" placeholder="-" onchange="Sales.calcRow(${idx})"></td>
        <td><input type="number" class="form-control form-control-sm d-qtyC number" data-field="child_qty" placeholder="-" onchange="Sales.calcRow(${idx})"></td>
        <td><input type="number" class="form-control form-control-sm d-priC number" data-field="child_price" placeholder="-" onchange="Sales.calcRow(${idx})"></td>
        <td><input type="number" class="form-control form-control-sm d-sur number" data-field="surcharge" placeholder="-" onchange="Sales.calcRow(${idx})"></td>
        <td><input type="number" class="form-control form-control-sm d-disc number" data-field="discount" placeholder="-" onchange="Sales.calcRow(${idx})"></td>
        ...
        <td class="text-center align-middle"><i class="fa-solid fa-times text-danger" style="cursor:pointer" onclick="Sales.removeRow(${idx})"></i></td>
            `;
     ...
     */
    function getExtendedCustomerData() {
      try {
     ...
     */
    function updateCustomerTab(custData) {
    ...
    /**
     * Initialize Sales Module - Expose functions to window.Sales namespace
     * ✅ Cách 2: Namespace Bridge (Transitional to pure ES6)
     * 
     * IMPORTANT: These are SALES-SPECIFIC implementations, different from Operator role
     * - loadBookingToUI: Uses customerData parameter, Sales form structure
     * - calcGrandTotal: Uses .d-total selectors, Sales calculations
     * - calcRow: Uses .d-pri/.d-priC selectors, Sales-specific logic
     * - addDetailRow: Sales form HTML template structure
     * - ALL others: Either unique to Sales or shared with Operator
     * 
     * Usage in HTML:
     * <button onclick="Sales.saveForm()"></button>
     * <a onclick="Sales.updateDeposit()"></a>
     * <input onchange="Sales.findCustByPhone()">
     */
    function initSalesModule() {
      console.log('[SalesModule] 🚀 Initializing Sales Module (Namespace Approach)...');
      console.log('📋 [SalesModule] Duplicate functions override: loadBookingToUI, calcGrandTotal, calcRow, addDetailRow');
      
      // Create namespace to avoid naming conflicts between roles
      // Example: Sales.updateBkStatus vs Operator.updateBkStatus (will be created in future OperatorModule)
      window.Sales = {
        // ═══════════════════════════════════════════════════════════════════════════
        // ROLE-SPECIFIC FUNCTIONS (Sales-only implementations)
        // ═══════════════════════════════════════════════════════════════════════════
        // These use Sales form structure (.d-pri, .d-total, .d-totalSales)
        // Different from Operator versions in logic_operator.js
        ConfirmationModule,         // ⭐ Sales-specific confirmation dialog module
        loadBookingToUI,           // ⭐ Sales version with customerData parameter
        addDetailRow,              // ⭐ Sales form structure
        calcRow,                   // ⭐ Sales calculation (.d-pri/.d-priC)
        calcGrandTotal,            // ⭐ Sales calculations (.d-total)
        
        // ═══════════════════════════════════════════════════════════════════════════
        // SHARED FUNCTIONS (Same logic as Operator, can be reused)
        // ═══════════════════════════════════════════════════════════════════════════
        removeRow,                 // Generic row removal
        copyRow,                   // Generic row copy
        findCustByPhone,           // Same search logic everywhere
        onTypeChange,              // Shared dropdown logic
        onLocationChange,          // Shared dropdown logic
        updateLocationList,        // Shared location fill
        updateServiceNameList,     // Shared service fill
        
        // ═══════════════════════════════════════════════════════════════════════════
        // UNIQUE TO SALES (No Operator equivalent)
        // ═══════════════════════════════════════════════════════════════════════════
        sortDetailsData,           // Sort details by type + date
        autoFillRowData,           // Auto-fill row from selection
        getFormData,               // Extract form data for save
        getExtendedCustomerData,   // Get extended customer info
        
        // ═══════════════════════════════════════════════════════════════════════════
        // DROPDOWNS & MODALS (Sales-specific actions)
        // ═══════════════════════════════════════════════════════════════════════════
        createConfirmation,        // Open confirmation dialog
        updateBkStatus,            // Update booking status dropdown
        updateDeposit,             // Update deposit amount
        autoSetOrCalcDate,         // Auto-set or calculate dates
        
        // ═══════════════════════════════════════════════════════════════════════════
        // CUSTOMER & TEMPLATE MANAGEMENT (Unique to Sales)
        // ═══════════════════════════════════════════════════════════════════════════
        updateCustomerTab,         // Update customer information tab
        prepareCreateCustomer,     // Prepare new customer creation dialog
        
        // ═══════════════════════════════════════════════════════════════════════════
        // API FUNCTIONS (Save, Delete, Contracts, Templates)
        // ═══════════════════════════════════════════════════════════════════════════
        deleteForm,                // Delete booking
        createContract,            // Create contract from booking
        saveCurrentTemplate,       // Save as booking template
        checkAndLoadTemplate,      // Load template by name
        askToLoadTemplate,         // Ask user to load template
        processAndFillTemplate,    // Process and fill template data
        requestDeleteFile,         // Delete contract file
        handleSaveCustomer,        // Save customer info
        saveBatchDetails,          // Batch save multiple rows
        handleAggClick             // Handle aggregated row click for batch editing

      };
      
      console.log('[SalesModule] ✅ All functions exposed to window.Sales namespace', {
        roleSpecific: 4,           // loadBookingToUI, addDetailRow, calcRow, calcGrandTotal
        shared: 7,                 // removeRow, copyRow, findCustByPhone, onTypeChange, etc.
        uniqueToSales: 7,          // fillFormFromSearch, sortDetailsData, updateCustomerTab, prepareCreateCustomer, etc.
        dropdownsModals: 4,        // createConfirmation, updateBkStatus, updateDeposit, autoSetOrCalcDate
        apiFunctions: 5,           // saveForm, deleteForm, createContract, handleSaveCustomer, requestDeleteFile
        contractTemplate: 5,       // saveCurrentTemplate, checkAndLoadTemplate, askToLoadTemplate, processAndFillTemplate
        other: 1,                  // handleAggClick
        totalExposed: 33,
        pattern: 'Namespace: window.Sales.*',
        htmlUsage: 'onclick="Sales.saveForm()" or onchange="Sales.findCustByPhone()"'
      });
      
      // Backward compatibility: Some code might call these directly
      // (Will be removed in Phase 3 - Pure ES6 refactor)
      window.fillFormFromSearch = fillFormFromSearch;
    ...
    // ═══════════════════════════════════════════════════════════════════
    // OPTIONAL: Export individual functions for testing/imports
    // ═══════════════════════════════════════════════════════════════════
    // Export only initSalesModule - login_module.js calls this on role detection
    // All functions are exposed via window.Sales namespace inside initSalesModule
    // ═══════════════════════════════════════════════════════════════════
    
    export { initSalesModule };
