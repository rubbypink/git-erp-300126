...
/**
 * =========================================================================
 * ScrollHeaderManager — Auto-hide Header/Footer on Scroll (Mobile)
 * Purpose: Tối ưu không gian hiển thị trên mobile bằng cách ẩn header/footer
 *          khi người dùng cuộn xuống và hiện lại khi cuộn lên.
 * =========================================================================
 */
class ScrollHeaderManager {
  ...
  /**
   * Khởi tạo listener cho sự kiện scroll
   */
  init() {
    if (this.#initialized) return this;

    // Chỉ kích hoạt trên mobile (max-width: 991px)
    ...
    // Lắng nghe trên .app-content hoặc .tab-content (vùng chứa nội dung chính của ERP)
    // Nếu không tìm thấy, fallback về window
    const scrollContainer = document.querySelector('.app-content') || document.querySelector('.tab-content') || window;

    scrollContainer.addEventListener('scroll', (e) => this.#onScroll(e), {
      signal,
      passive: true,
    });

    console.log('[ScrollHeaderManager] ✅ Initialized — Auto-hide header/footer active');
    return this;
  }

  /**
   * Hủy bỏ các listener và dọn dẹp
   */
  destroy() {
    this.#abortController?.abort();
    this.#abortController = null;
  ...
  /**
   * Xử lý sự kiện scroll với RequestAnimationFrame để tối ưu performance
   */
  #onScroll(e) {
    if (!this.#ticking) {
      window.requestAnimationFrame(() => {
        this.#handleScrollLogic(e);
        this.#ticking = false;
      });
      this.#ticking = true;
    }
  ...
  /**
   * Logic xác định hướng scroll và ẩn/hiện UI
   */
  #handleScrollLogic(e) {
    try {
      ...
      // 1. Luôn hiện lại khi ở đầu trang
      if (scrollTop <= 0) {
        this.#showHeader();
      ...
      // 2. Kiểm tra ngưỡng kích hoạt (threshold)
      if (Math.abs(scrollTop - this.#lastScrollTop) <= this.#scrollThreshold) {
        return;
      }

      // 3. Xác định hướng scroll
      ...
      console.error('[ScrollHeaderManager] Error in scroll logic:', err);
    }
  }

  /**
   * Thêm class CSS để ẩn header và footer
   */
  #hideHeader() {
    const header = document.querySelector('.app-header');
    const footer = document.querySelector('.erp-taskbar');
  ...
  /**
   * Xóa class CSS để hiện lại header và footer
   */
  #showHeader() {
    const header = document.querySelector('.app-header');
    const footer = document.querySelector('.erp-taskbar');
...
/** @type {ScrollHeaderManager} Singleton instance cho việc ẩn/hiện header */
const ScrollManager = new ScrollHeaderManager();
// Chỉ khởi tạo ScrollManager nếu là thiết bị touch (mobile/tablet)
if (TouchGestureAdapter.isTouch) {
  ScrollManager.init();
}
...
// EXPORT
// =========================================================================
export { MobileEvent, ScrollManager };
