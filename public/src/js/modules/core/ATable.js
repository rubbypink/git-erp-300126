...
export default class ATable {
  ...
  async init(data = [], colName = null) {
    try {
      ...
      // 2. Kiểm tra xem cấu trúc dữ liệu mới có trùng với cấu trúc hiện tại không
      let isStructureChanged = false;
      if (colName && colName !== this.options.colName) {
        ...
        if (currentKeys.length === 0) {
          ...
          // So sánh các keys mới với các keys hiện có trong fieldConfigs
      ...
      // 3. Nếu cấu trúc trùng khớp hoàn toàn: Không reset fieldConfigs, chỉ render lại data
      if (!isStructureChanged) {
        this.state.fullData = normalizedData;
        this.state.filteredData = [...this.state.fullData];
        this.render(true, true); // isUpdate = true, skipHeaderUpdate = true
        return this;
      }

      // 4. Nếu cấu trúc khác: Thực hiện reset và render như bình thường
      ...
      // 5. Tự động nhận diện colName nếu không có
      if (!this.options.colName) {
        this._autoDetectColName(data);
      }

      // 6. Xử lý colName đặc biệt (_by_)
  ...
  render(isUpdate = false, skipHeaderUpdate = false) {
    ...
    if (isUpdate && wrapper) {
      try {
        // 1. Cập nhật thead (để đổi icon sort hoặc cấu trúc cột) - Bỏ qua nếu skipHeaderUpdate = true
        if (!skipHeaderUpdate) {
