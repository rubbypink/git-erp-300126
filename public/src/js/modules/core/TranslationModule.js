...
class TranslationModule {
  ...
  async _loadFromCache() {
    try {
      const isReady = await localDB.initDB();
      if (!isReady) return;
      ...
      if (dictRaw) {
        try {
          ...
          console.warn('⚠️ Lang: Parse Dict Error', e);
        }
      }
      ...
      if (schemaRaw) {
        try {
          ...
          console.warn('⚠️ Lang: Parse Schema Error', e);
        }
      }
    } catch (e) {
  ...
  async _syncRemote() {
    ...
    try {
      ...
      if (data && data[LANG_DOC] && data[LANG_DOC][this.currentLang]) {
        ...
        // await A.DB.local.setMeta(META_KEY_DICT, JSON.stringify(this.dict));
        // FIX: Đảm bảo object có field 'id' để cleanDataForFirestore không lọc sạch
        const syncData = { 
          id: LANG_DOC, 
          [this.currentLang]: this.dict 
        };
        await localDB.put('app_config/general/settings', syncData);
      }
    } catch (error) {
  ...
  async syncFromSchema() {
    ...
    try {
      ...
      if (addedCount > 0) {
        ...
        // await A.DB.local.setMeta(META_KEY_SCHEMA, JSON.stringify(schemaDict));
  ...
  async update(key, value) {
    ...
    // FIX: Đảm bảo object có field 'id'
    await localDB.put('app_config/general/settings', { id: LANG_DOC, [this.currentLang]: { [key]: value } });
    ...
    try {
      ...
      const docRef = doc(A.DB.db, ROOT_COL, GENERAL_DOC, SETTINGS_COL, LANG_DOC);
