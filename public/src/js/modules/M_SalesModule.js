  ...
  static UI = {
    ...
    loadBookingToUI: (bkData, customerData, detailsData) => {
      ...
      try {
        ...
        if (customerData) {
          SalesModule.Logic.findCustByPhone(customerData);
        }
    ...
    addDetailRow: (data = null) => {
      try {
        ...
        tr.innerHTML = `
          ...
          <td>
            <select class="form-select form-select-sm" data-field="hotel_name" onchange="SalesModule.Logic.onLocationChange(${idx})">
              <option value="">-</option>
            </select>
          </td>
          <td>
          ...
          <td><input type="date" class="form-control form-control-sm" data-field="check_in" onchange="SalesModule.Logic.autoSetOrCalcDate(this.value, $('[data-field=\'check_out\']', $('#row-${idx}')))" style="cursor:pointer"></td>
          ...
          <td><input type="number" class="form-control form-control-sm number" data-field="unit_price" placeholder="-"></td>
  ...
  static Logic = {
    ...
    copyBooking: (bkId) => {
      try {
        ...
        const sourceDetails = allDetails[bkId];
        ...
        // Giả định ngày bắt đầu của booking nguồn là check_in nhỏ nhất trong details
        const parseDate = (dStr) => (dStr instanceof Date ? dStr : new Date(dStr));
        ...
        detailsArr.forEach((row) => {
          ...
          // Logic cập nhật số lượng dựa theo input mới (tương tự processAndFillTemplate)
          if (!['Xe', 'HDV', 'Tàu', 'Ca nô'].includes(type)) {
            finalQtyA = type === 'Phòng' ? Math.ceil(newAdult / 2) : newAdult;
            finalQtyC = newChild;
          }
