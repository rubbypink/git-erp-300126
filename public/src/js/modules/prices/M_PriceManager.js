...
export default class PriceManager {
  ...
  async init(containerId = 'tab-price-pkg') {
    try {
      ...
      // /* OVERLAP: Sub-modules tự load master data của riêng mình */
  ...
  attachEvents() {
    try {
      ...
      /* OVERLAP: Các bộ lọc này đã có trong từng module con
      const selSupplier = getE('pm-supplier-select');
      if (selSupplier) {
        selSupplier.addEventListener('change', (e) => {
          const supplierId = e.target.value;
          this.updateFilter({ supplierId });
          if (this.state.activeModule === 'hotel' && this.controllers.hotel?.selSupplier) {
            this.controllers.hotel.selSupplier.value = supplierId;
            this.controllers.hotel.selSupplier.dispatchEvent(new Event('change'));
          } else if (this.state.activeModule === 'service' && this.controllers.service?.selSupplier) {
            this.controllers.service.selSupplier.value = supplierId;
            this.controllers.service.selSupplier.dispatchEvent(new Event('change'));
          }
        });
      }

      const selHotel = getE('pm-hotel-select');
      if (selHotel) {
        selHotel.addEventListener('change', (e) => {
          const hotelId = e.target.value;
          this.updateFilter({ hotelId });
          if (this.state.activeModule === 'hotel' && this.controllers.hotel?.selHotel) {
            this.controllers.hotel.selHotel.value = hotelId;
            this.controllers.hotel.selHotel.dispatchEvent(new Event('change'));
          }
        });
      }

      const selYear = getE('pm-year-select');
      if (selYear) {
        selYear.value = this.state.currentFilter.year;
        selYear.addEventListener('change', (e) => {
          const year = e.target.value;
          this.updateFilter({ year });
          if (this.state.activeModule === 'hotel' && this.controllers.hotel?.selYear) {
            this.controllers.hotel.selYear.value = year;
          } else if (this.state.activeModule === 'service' && this.controllers.service?.selYear) {
            this.controllers.service.selYear.value = year;
          }
        });
      }

      const btnQuery = getE('pm-btn-query');
      if (btnQuery) btnQuery.addEventListener('click', () => this.handleQuickQuery());
      */
  ...
  /* OVERLAP: Logic truy vấn nhanh nên nằm trong module Hotel hoặc Service
  async handleQuickQuery() {
    try {
      const start = getVal('pm-query-start');
      const end = getVal('pm-query-end');
      const hotelId = this.state.currentFilter.hotelId;

      if (!start || !end) return logA('Vui lòng chọn khoảng ngày', 'warning', 'toast');

      if (this.state.activeModule === 'hotel' && hotelId) {
        this.toggleLoading(true);
        const CostManager = (await import('./M_CostManager.js')).default;
        const result = await CostManager.getHotelPrice(hotelId, start, end, 'base');

        if (result.success) {
          setDisplay('pm-quick-info', true);
          setText('pm-min-price-val', formatNumber(result.price));
          logA(`Giá bình quân: ${formatNumber(result.price)}`, 'success', 'toast');
        } else {
          logA(result.error || 'Không tìm thấy giá', 'warning', 'toast');
        }
      } else {
        logA('Tính năng truy vấn nhanh hiện hỗ trợ cho Khách sạn', 'info', 'toast');
      }
    } catch (error) {
      Opps(error, 'PriceManager.handleQuickQuery');
    } finally {
  ...
  */
  ...
  async loadMasterData() {
    ...
    try {
      ...
      fillSelect(
        'pm-supplier-select',
        this.state.masterData.suppliers.map((s) => ({ id: s.id, name: s.name })),
        'Chọn Nhà cung cấp'
      );
      fillSelect(
        'pm-hotel-select',
        this.state.masterData.hotels.map((h) => ({ id: h.id, name: h.name })),
        'Chọn Khách sạn'
      );
    } catch (error) {
  ...
  async onPriceChanged(moduleType, payload) {
    ...
    try {
      ...
      await A.DB.saveRecord('notifications', notification);
    } catch (error) {
