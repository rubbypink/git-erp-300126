...
class TourPriceController {
  ...
  _attachEvents() {
    // Nút mở Modal
    getE('tp-btn-open-base')?.addEventListener('click', () => this.openBasePriceModal());

    // Select Tour
    getE('tp-tour-select')?.addEventListener('change', (e) => this.loadData(e.target.value));

    // Nút Xuất PDF
    getE('tp-btn-export')?.addEventListener('click', () => this.exportPDF());

    // Nút Tải Ảnh
    getE('tp-btn-export-image')?.addEventListener('click', () => this.exportImage());

    // Nút Toggle Format (Logo)
    getE('tp-logo-toggle')?.addEventListener('click', () => this.toggleFormat());

    // Zoom Events
    getE('tp-btn-zoom-in')?.addEventListener('click', () => this.changeZoom(0.05));
    getE('tp-btn-zoom-out')?.addEventListener('click', () => this.changeZoom(-0.05));
    getE('tp-zoom-text')?.addEventListener('click', () => this.changeZoom(0, true));
  }
  ...
  async saveData() {
    try {
      ...
      if (res.success) {
        ...
        await A.DB.local.put('selling_prices', {
          id: data.id,
  ...
  async loadData(id) {
    ...
    try {
      ...
      // 2. Nếu không có, thử lấy từ IndexedDB (selling_prices)
      if (!data) {
        const localData = await A.DB.local.get('selling_prices', id);
        if (localData) {
          data = {
            id: localData.id,
  ...
  async deletePrice() {
    try {
      ...
      if (res.success) {
        ...
        await A.DB.local.delete('selling_prices', id);
