...
export default class ServicePriceController {
  ...
  initTable() {
    ...
    // Định nghĩa cấu hình cột thủ công vì schema của collection không khớp với từng dòng dịch vụ
    this.table.state.fieldConfigs = {
  ...
  async loadTableData() {
    ...
    try {
      ...
      if (tableData && tableData.items) {
        this.table.updateData(normalizeList(tableData.items));
