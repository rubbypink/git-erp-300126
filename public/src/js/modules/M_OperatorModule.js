    ...
    addDetailRow: (data = null) => {
      try {
        ...
        tr.innerHTML = `
          ...
          <td><input type="date" class="form-control form-control-sm p-1" data-field="check_in" onchange="Op.Logic.autoSetOrCalcDate(this.value, $('[data-field=\\'check_out\\']', $('#row-${idx}')))"></td>
          ...
          <td><input type="number" class="form-control form-control-sm text-center fw-bold number-only" data-field="adults" value="1" onchange="Op.Logic.calcRow(${idx})"></td>
          <td><input type="text" class="form-control form-control-sm fw-bold text-end bg-warning bg-opacity-10 number-only" data-field="cost_adult" onchange="Op.Logic.calcRow(${idx})" placeholder="0"></td>
          <td><input type="number" class="form-control form-control-sm text-center number-only" data-field="children" value="0" onchange="Op.Logic.calcRow(${idx})"></td>
          <td><input type="text" class="form-control form-control-sm text-end bg-warning bg-opacity-10 number-only" data-field="cost_child" onchange="Op.Logic.calcRow(${idx})" placeholder="0"></td>
          <td><input type="text" class="form-control form-control-sm text-end small text-muted number-only" data-field="surcharge" onchange="Op.Logic.calcRow(${idx})" placeholder="0"></td>
          <td><input type="text" class="form-control form-control-sm text-end small text-muted number-only" data-field="discount" onchange="Op.Logic.calcRow(${idx})" placeholder="0"></td>
          ...
          <td><input type="text" class="form-control form-control-sm number-only text-end text-success fw-bold" data-field="paid_amount" onchange="Op.Logic.calcRow(${idx}); typeof Op.DB.syncTransactionForPaidAmount === 'function' && Op.DB.syncTransactionForPaidAmount(${idx})" placeholder="0"></td>
  ...
  static Logic = {
    ...
    onSupplierChange: async (idx) => {
      try {
        ...
        if (!service || !useDate || !type || !window.A?.PriceManager) return;
  ...
  static DB = {
    ...
    syncRow: async (sourceRow = null) => {
      ...
      try {
        ...
        for (const tr of rows) {
          ...
          const bkDetail = await window.A?.DB?.db?.collection('booking_details').doc(sid).get();
          if (bkDetail?.exists) {
            await window.A?.DB?._syncOperatorEntry(bkDetail.data());
            const newSnap = await window.A?.DB?.db?.collection('operator_entries').doc(sid).get();
            if (newSnap?.exists) {
    ...
    syncTransactionForPaidAmount: async (idx) => {
      try {
        ...
        const newTxRef = window.A?.DB?.db?.collection('transactions').doc();
        const fundRef = window.A?.DB?.db?.collection('fund_accounts').doc(selectedFundId);

        const result = await window.A?.DB?.db?.runTransaction(async (transaction) => {
          ...
          // const newBalance = Number(String(currentBalance).replace(/[^0-9]/g, '')) - diffAmount;
          // L._(`[SupplierPayment] Balance: ${currentBalance} -> ${newBalance} : ${diffAmount}`);
        ...
        if (result) {
          ...
          window.A?.DB?._updateAppDataObj('transactions', newTransaction);

          // DEACTIVATED: Việc cập nhật fund_accounts sẽ do Cloud Functions thực hiện
          /*
          const updatedFundAccount = {
            ...window.APP_DATA.fund_accounts[selectedFundId],
            balance: newBalance,
          };
          window.A?.DB?._updateAppDataObj('fund_accounts', updatedFundAccount);
          */

          logA(`✅ Đã tạo phiếu chi. Số dư tài khoản sẽ được hệ thống cập nhật tự động.`, 'success');

          if (window.A?.NotificationManager) {
            window.NotificationManager.sendToAdmin('Thanh toán tự động', `Đã tạo phiếu chi ${formatNumber(diffAmount)} từ tài khoản ${fundAccounts[selectedFundId].name}`);
          }
        }
      } catch (error) {
