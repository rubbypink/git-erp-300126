...
class Op {
  ...
  static UI = {
      ...
      try {
        ...
        for (const rowData of dataList) {
          ...
          tr.innerHTML = `
            ...
            <td>
              <select class="smart-select form-select form-select-sm text-wrap" data-source="hotels" data-searchable="true" data-field="hotel_name" data-onchange="Op.Logic.onLocationChange(${idx})">
                <option value="">-</option>
              </select>
            </td>    
          ...
          if (rowData) {
            ...
            if (typeof Op.UI.updateServiceSelect === 'function') {
              ...
              // Vì updateServiceSelect có thể thay đổi options, ta setVal sau đó
              setTimeout(() => setVal('[data-field="service_name"]', rowData.service_name, tr), 50);
            } else {
    ...
    updateServiceSelect: (idx) => {
      try {
        ...
        const elName = tr.querySelector('[data-field="service_name"]');
        if (!elName) return;
        ...
        console.error('Op.UI.updateServiceSelect Error:', e);
      }
    },
  ...
  static Logic = {
    ...
    onLocationChange: (idx, resetName = true) => {
      try {
        const tr = getE(`row-${idx}`);
        if (tr && getVal('[data-field="service_type"]', tr) === 'Phòng') {
          Op.UI.updateServiceSelect(idx);
          if (resetName) setVal('[data-field="service_name"]', '', tr);
        }
      } catch (e) {
        console.error('Op.Logic.onLocationChange Error:', e);
      }
    },
