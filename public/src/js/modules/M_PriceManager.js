...
export default class PriceManager {
  ...
  static async getHotelPrice(hotelIdentifier, checkInDate, checkOutDate, roomIdentifier, rateTypeId = 'base', packageId = 'base') {
    try {
      ...
      return await this._withTimeout(this._executeGetHotelPrice(hotelIdentifier, checkInDate, checkOutDate, roomIdentifier, rateTypeId, packageId), TIMEOUT_CONFIG.GET_HOTEL_PRICE, 'getHotelPrice');
    } catch (error) {
  ...
  static async _executeGetHotelPrice(hotelIdentifier, checkInDate, checkOutDate, roomIdentifier, rateTypeId = 'base', packageId = 'base') {
    ...
    const [priceDoc, roomsList, periodsList] = await this._withTimeout(Promise.all([this._fetchPriceSchedule(hotelIdentifier, year), this._fetchHotelRooms(hotelIdentifier), this._fetchMasterPeriods()]), TIMEOUT_CONFIG.FETCH_OPERATIONS, 'Data fetching');
  ...
  static async _executeGetServicePrice(serviceName, useDate, supplierId = null) {
    ...
    // TRƯỜNG HỢP 2: Không chỉ định Supplier -> Quét tất cả bảng giá Active trong năm
      ...
      const querySnap = await firebase.firestore().collection(DB_PATHS.SERVICE_SCHEDULES).where('info.year', '==', year).where('info.status', '==', 'actived').get();
  ...
  static async _fetchPriceSchedule(hotelIdentifier, year) {
    ...
    let query = colRef.where('info.year', '==', year).where('info.status', '==', 'actived');
  ...
  static async _fetchHotelRooms(hotelIdentifier) {
    try {
      ...
      if (!hotelDoc.exists) {
        L._('Fallback: Tìm hotel bằng id  ko thành công.');
        // Fallback: Nếu không tìm thấy bằng ID, thử query bằng field 'name'
        const querySnapshot = await firebase.firestore().collection(DB_PATHS.HOTELS).where('name', '==', hotelIdentifier).limit(1).get();
      ...
      L._(
        'Rooms fetched:',
        roomsSnap.docs.map((d) => ({ id: d.id, ...d.data() }))
      );
      return roomsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
    } catch (e) {
  ...
  static async _fetchMasterPeriods() {
    ...
    try {
      ...
      L._(
        'Fetched periods from DB:',
        periods.docs.map((d) => ({ id: d.id, ...d.data() }))
      );
      ...
      L._('Lỗi fetch periods, dùng fallback:', e);
      return [{ id: 'standard_season', name: 'Mùa Hè', from: '01/04', to: '30/08' }];
    }
  }
