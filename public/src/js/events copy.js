...
async function setupStaticEvents() {
    ...
    if (CURRENT_USER.role === 'op' || btnHotelPriceTbl) {       
        btnHotelPriceTbl.addEventListener('click', (e) => {
            ...
            const pc = new HotelPriceController('dynamic-modal-full-body');
    ...
    if (btnBatchCreateData) {
        btnBatchCreateData.addEventListener('click', (e) => {
            ...
            if (FirestoreDataTableManager) {
                log('[BATCH CREATE DATA] Khởi tạo FirestoreDataTableManager...');
                const modal = document.querySelector('at-modal-full');
                new FirestoreDataTableManager('dynamic-modal-full-body');
                modal.setFooter(false);
                modal.show();
            } else {
                console.warn('FirestoreDataTableManager or batchCreateData method is not available.');
            }
        });
    }
    ...
    onEvent(numberInputSelector, 'input', function(e, target) {
        ...
        target._debounceTimer = setTimeout(function() {
            ...
            if (tr && tr.id && typeof calcRow === 'function') {
                if(!CURRENT_CTX_ROW) CURRENT_CTX_ROW = tr;
...
function setupContextMenu(elementOrId = 'detail-tbody') {
    ...
    if (CURRENT_CTX_ROW && CURRENT_ROW_DATA) {
        const res = await A.DB.saveRecord(collection, 
            CURRENT_ROW_DATA
        );
        if (res.success) {
