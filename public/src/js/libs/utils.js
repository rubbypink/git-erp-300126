...
/**
 * Helper: Xử lý mảng lớn không gây treo trình duyệt (Time Slicing / Chunking)
 * @param {Array} array - Mảng dữ liệu lớn cần xử lý
 * @param {Function} processFn - Hàm xử lý từng phần tử
 * @param {Number} chunkSize - Số phần tử xử lý trong 1 Frame (Khuyên dùng: 50 - 200)
 * @returns {Promise} - Trả về mảng kết quả sau khi xử lý xong toàn bộ
 */
const batchRunFnAsync = (array, processFn, chunkSize = 100) => {
    return new Promise((resolve, reject) => {
        if (!array || array.length === 0) return resolve([]);
        let index = 0;
        const total = array.length;
        const results = [];
        // Hàm đệ quy an toàn chạy trong từng Frame
        const processChunk = () => {
            try {
                // Tính toán điểm dừng của frame hiện tại
                const end = Math.min(index + chunkSize, total);
                // Chạy vòng lặp NHỎ trong frame này
                for (; index < end; index++) {
                    const res = processFn(array[index], index);
                    if (res !== undefined && res !== null) {
                        results.push(res);
                    }
                }
                // Kiểm tra xem đã xử lý hết mảng chưa?
                if (index < total) {
                    // Chưa hết -> Đặt lịch chạy tiếp chunk sau ở Frame tiếp theo
                    requestAnimationFrame(processChunk);
                } else {
                    // Xong xuôi -> Trả về kết quả
                    resolve(results);
                }
            } catch (error) {
                // Rất quan trọng: Bắt lỗi để không bị treo vĩnh viễn Promise
                reject(error);
            }
        };
        // Kích hoạt phát súng đầu tiên
        requestAnimationFrame(processChunk);
    });
};
