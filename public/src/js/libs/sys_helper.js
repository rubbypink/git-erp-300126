...
}; // Thêm hàm helper này vào object SYS (hoặc hàm toàn cục của bạn)
SYS.renderConsoleLog = function (data, depth = 0) {
    // 1. Bảo vệ chống lặp đệ quy vô hạn (Circular Reference) hoặc quá sâu gây crash
    if (depth > 15) return `<span class="text-muted fst-italic">[Đã đạt giới hạn độ sâu hiển thị]</span>`;

    // 2. Xử lý các kiểu dữ liệu cơ sở (Primitive types) và Null/Undefined
    if (data === null) return `<span class="text-warning fw-bold">null</span>`;
    if (data === undefined) return `<span class="text-warning fw-bold">undefined</span>`;
    if (typeof data === 'function') return `<span class="text-muted fst-italic">function() {...}</span>`;
    if (typeof data !== 'object') {
        let colorClass = typeof data === 'string' ? 'text-success' : 'text-warning';
        let displayVal = typeof data === 'string' ? `"${data}"` : data;
        return `<span class="${colorClass}">${displayVal}</span>`;
    }

    // 3. Xử lý các object đặc biệt (Map, Set, Date)
    if (data instanceof Date) return `<span class="text-info">${data.toISOString()}</span>`;
    if (data instanceof Map) data = Object.fromEntries(data); // Ép Map về Object phẳng
    if (data instanceof Set) data = Array.from(data); // Ép Set về Array phẳng

    // 4. Trích xuất keys an toàn
    const isArray = Array.isArray(data);
    const keys = Object.keys(data);

    // Nếu rỗng, in ra [] hoặc {}
    if (keys.length === 0) {
        return `<span class="text-muted fw-bold">${isArray ? '[]' : '{}'}</span>`;
    }
    ...
    // Tối ưu UI: Mở sẵn các nhánh ở độ sâu thấp, tự động đóng nhánh sâu để tránh rác màn hình
    const isOpen = depth < 1 ? 'open' : '';

    let html = `
      <details style="margin-left: 15px;" ${isOpen}>
          <summary class="fw-bold text-primary" style="cursor: pointer; user-select: none; display: list-item;">
              ${typeLabel} ${bracketOpen}
          </summary>
          <div style="border-left: 1px dashed #dee2e6; margin-left: 6px; padding-left: 15px;">`;

    for (let key of keys) {
        html += `<div class="mb-1" style="word-break: break-all;">
                  <span class="text-secondary">"${key}"</span>: ${SYS.renderConsoleLog(data[key], depth + 1)}
               </div>`;
    }

    html += `       </div>
          <div class="fw-bold text-primary" style="margin-left: 6px;">${bracketClose}</div>
      </details>`;
