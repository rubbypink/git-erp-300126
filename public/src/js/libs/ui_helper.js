...
class Resizable {
    ...
    resizeStart(e) {
        ...
        if (e.target.classList.contains('modal-dialog-centered')) e.target.classList.remove('modal-dialog-centered');
...
class WindowMinimizer {
    ...
    minimize() {
        ...
        // Thu gom và ẩn tất cả các backdrop hiện tại của Bootstrap
        this.hiddenBackdrops = [];
        ...
        // ★ FIX TRÀN TEXT: max-width 220px, cho phép xuống hàng (white-space: normal), canh trái
        this.taskItem.style.cssText = 'position: relative; overflow: visible !important; padding: 0.4rem 2.2rem 0.4rem 0.6rem; cursor: pointer; display: inline-flex; align-items: center; max-width: 220px; white-space: normal; text-align: left; font-size: 0.8rem; line-height: 1.3; border-radius: 6px;';

        // Cấu trúc lại Icon và Text để lúc xuống hàng text không đè lên Icon
        this.taskItem.innerHTML = `<i class="fa-solid fa-window-restore me-2 fs-6"></i><span style="flex: 1;">${title}</span>`;
        ...
        closeBtn.style.cssText = [
            ...
            'border: 2px solid #fff', // Thêm viền trắng mỏng để tách biệt hẳn với cái nền vàng
        ].join(';');
        closeBtn.innerHTML = '<i class="fa-solid fa-xmark"></i>';

        this.taskItem.addEventListener('mouseenter', () => (closeBtn.style.opacity = '1'));
        this.taskItem.addEventListener('mouseleave', () => (closeBtn.style.opacity = '0'));

        // Click X → đóng Bootstrap modal
        closeBtn.addEventListener('click', (e) => {
            e.stopPropagation();

            // Phục hồi DOM để hàm hide() của Bootstrap dọn dẹp đúng chuẩn
            this.target.style.display = this.oldDisplay || '';
            this.target.classList.add('show');
            if (this.hiddenBackdrops) {
                this.hiddenBackdrops.forEach((b) => (b.style.display = ''));
            }
     ...
     */
    restore() {
        // 1. Hiện lại cửa sổ & Phục hồi lớp chặn Body
        this.target.style.display = this.oldDisplay;
        this.target.classList.add('show'); // Gắn lại class hiển thị
        document.body.classList.add('modal-open'); // Khóa cuộn trang chủ

        // ★ FIX BACKDROP: Hiện lại các màn sương mù mờ phía sau
        if (this.hiddenBackdrops) {
            this.hiddenBackdrops.forEach((b) => (b.style.display = ''));
            this.hiddenBackdrops = [];
        }
        ...
        if (this.hadCenteredClass && this.removeCenteredClass) {
            ...
            if (modalDialog && !modalDialog.classList.contains('modal-dialog-centered')) {
                // Không bắt buộc phải restore vì sau khi kéo thả thì không cần center nữa
            }
        }
