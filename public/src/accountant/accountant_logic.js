/**
 * ACCOUNTANT LOGIC - Firebase V9 (Modular)
 *
 * Cập nhật sang Firebase V9 Modular để đồng bộ hoàn toàn với hệ thống.
 * Sử dụng DBManager để tương tác database.
 */

import { collection, getDocs, query, orderBy, limit, doc, getDoc, writeBatch, Timestamp } from 'firebase/firestore';
import DB_MANAGER from '/src/js/modules/db/DBManager.js';

/**
 * HELPER: Lấy Firestore DB instance từ DBManager
 */
const getDB = () => {
  const db = DB_MANAGER.db;
  if (!db) {
    throw new Error('❌ Lỗi: DBManager chưa được khởi tạo!');
  }
  return db;
};
...
export async function getNewData() {
  ...
  try {
    const fetchCollection = async (colName, cacheKey) => {
      ...
      L._(`- Đã tải ${dataList.length} docs từ [${colName}]`);
    };

    // Chạy song parallel để tối ưu tốc độ
    await Promise.all([fetchCollection('bookings', 'checkingBookings'), fetchCollection('transactions', 'checkingTransactions'), fetchCollection('fund_accounts', 'checkingFundAccounts')]);
...
export async function migrateBookingTransactions() {
  ...
  try {
    ...
    L._(`9 Trip Success: Hoàn tất Migrate! Đã tạo các transactions với updated_at chung.`);
