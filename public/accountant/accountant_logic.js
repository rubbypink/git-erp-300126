...
export async function getNewData() {
  ...
  try {
    const fetchCollection = async (colName, cacheKey) => {
      const snap = await db.collection(colName).orderBy('created_at', 'desc').limit(2000).get();
...
export async function migrateBookingTransactions() {
  ...
  try {
    ...
    L._(`9 Trip Success: Hoàn tát Migrate! Đã tạo các transactions với updated_at chung.`);
