...
- **Core Files:** - `public/src/js/libs/utils.js`: Chứa các hàm helper toàn cục.
  - `public/src/js/modules/db/DBManager.js`: Điểm tập trung tất cả các hàm xử lý dữ liệu giữa Client, indexedDB và Firestore.
...
- **CRUD Core Flow:** Luôn áp dụng nguyên tắc cốt lõi theo luồng dữ liệu: firestore -> indexeDB -> frontend (APP_DATA sẽ sớm bị loại bỏ để tối ưu ram) thông qua class DBManager: hàm #firestoreCRUD -> hàm #gatekeepSyncToLocal update localDB với thư viện hỗ trợ dexie -> tự động gọi hàm \_updateAppDataObj/removeAppDataObj cập nhật APP_DATA.
