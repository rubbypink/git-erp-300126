/**
 * Archive API (BẢN CHUẨN TỐI THƯỢNG)
 * Bao gồm Ép kiểu Type, Lọc rác và Chống xóa nhầm Batch
 */
...
const archiveOldData = onCall(async (request) => {
  ...
  try {
    ...
    // ĐÁNH DẤU CỜ CỨNG (Tuyệt đối an toàn chống Delete nhầm)
    ...
    for (const bookingDoc of bookingsSnap.docs) {
      ...
      const bookingId = String(bookingDoc.id); // ÉP KIỂU 1: ID phải luôn là String
      const bookingData = bookingDoc.data();
      const isAlreadyArchived = bookingDoc._isArchived === true;

      // =====================================
      // TẠO BỘ TÌM KIẾM ĐA KIỂU (Đỉnh cao lách lệch Type)
      // =====================================
      const searchIds = [bookingId]; // Tìm kiểu String trước
      const numId = Number(bookingId);
      if (!isNaN(numId)) {
        searchIds.push(numId); // Nếu ID là dạng số hợp lệ (VD "123"), tìm thêm cả dạng Số
      }

      // ÉP KIỂU 2: Check số dư khách hàng
      const balance = Number(bookingData.balance_amount) || 0;
      if (!isAlreadyArchived && balance > 0) continue;

      // Truy vấn Đa Kiểu cho các collection con
      ...
      // =====================================
      // BẮT ĐẦU ĐƯA VÀO BATCH
      // =====================================
      processedCount++;
      opCount += opsNeededForThisBooking;

      // CHỈ XÓA BẢN GỐC NẾU CHƯA ARCHIVE (Cờ _isArchived cứu mạng hệ thống)
      if (!isAlreadyArchived) {
        batch.set(db.collection('archived_bookings').doc(bookingId), bookingData);
        batch.delete(bookingDoc.ref);
      }

      // CHUẨN HÓA DỮ LIỆU CON: Ghi đè lại booking_id thành String chuẩn trước khi lưu kho
      opsSnap.forEach((doc) => {
        const data = doc.data();
        data.booking_id = bookingId; // Nắn lại Type
