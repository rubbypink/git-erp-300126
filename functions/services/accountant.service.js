/**
 * ═══════════════════════════════════════════════════════════════════════════
 * Accountant Service
 * Xử lý logic kế toán nâng cao
 * ═══════════════════════════════════════════════════════════════════════════
 */

const { logger } = require('firebase-functions');
const { getFirestore, admin } = require('../utils/firebase-admin.util');

/**
 * Chốt số dư tài khoản quỹ
 * Tính toán lại balance dựa trên các giao dịch từ lần chốt cuối cùng
 *
 * @param {string} accountId - ID của fund_account
 * @param {Object|null} start - Timestamp bắt đầu (nếu không có commit_date)
 * @param {Object|null} end - Timestamp kết thúc (mặc định server time)
 * @return {Promise<number>} Số dư mới sau khi chốt
 */
async function commitFundAccountLogic(accountId, start, end) {
  const db = getFirestore();
  const fundRef = db.collection('fund_accounts').doc(accountId);
  const serverNow = admin.firestore.Timestamp.now();
  const endTime = end ? (typeof end === 'string' ? admin.firestore.Timestamp.fromDate(new Date(end)) : end) : serverNow;

  try {
    return await db.runTransaction(async (t) => {
      const fundDoc = await t.get(fundRef);
      if (!fundDoc.exists) {
        throw new Error(`Tài khoản quỹ ${accountId} không tồn tại.`);
      }

      const fundData = fundDoc.data();
      let baseDate = fundData.commit_date;

      // Xác định baseDate
      if (!baseDate) {
        if (start) {
          baseDate = typeof start === 'string' ? admin.firestore.Timestamp.fromDate(new Date(start)) : start;
        } else {
          // Default: Tìm transaction sớm nhất
          const firstTrans = await db.collection('transactions').where('fund_source', '==', accountId).orderBy('transaction_date', 'asc').limit(1).get();

          if (!firstTrans.empty) {
            const d = firstTrans.docs[0].data().transaction_date;
            baseDate = typeof d === 'string' ? admin.firestore.Timestamp.fromDate(new Date(d)) : d;
          } else {
            baseDate = admin.firestore.Timestamp.fromDate(new Date('2000-01-01'));
          }
        }
      }

      // Query transactions
      // Lưu ý: transaction_date trong DB có thể là String (YYYY-MM-DD) hoặc Timestamp
      // Để an toàn, ta query theo logic: > baseDate và <= endTime
      const transQuery = db.collection('transactions').where('fund_source', '==', accountId).where('status', '==', 'Completed');

      const snapshot = await transQuery.get();

      let totalIn = 0;
      let totalOut = 0;
      const committedTransRefs = [];

      snapshot.forEach((doc) => {
        const data = doc.data();
        let tDate = data.transaction_date;

        // Convert to Timestamp for comparison if it's a string
        if (typeof tDate === 'string') {
          tDate = admin.firestore.Timestamp.fromDate(new Date(tDate));
        }

        if (tDate > baseDate && tDate <= endTime) {
          const amt = parseFloat(data.amount || 0);
          if (data.type === 'IN') totalIn += amt;
          else if (data.type === 'OUT') totalOut += amt;

          committedTransRefs.push(doc.ref);
        }
      });

      const balanceShift = totalIn - totalOut;
      const currentAmount = parseFloat(fundData.amount || fundData.balance || 0);
      const newAmount = currentAmount + balanceShift;

      // Update fund account
      t.update(fundRef, {
        amount: newAmount,
        balance: newAmount, // Đồng bộ cả 2 field nếu cần
        commit_date: endTime,
        updated_at: serverNow,
      });

      // Update transactions status to 'Commited'
      committedTransRefs.forEach((ref) => {
        t.update(ref, {
          status: 'Commited',
          committed_at: serverNow,
        });
      });

      logger.info(`✅ Committed account ${accountId}: ${currentAmount} -> ${newAmount} (Shift: ${balanceShift}, Trans: ${committedTransRefs.length})`);

      return { success: true, newBalance: newAmount };
    });
  } catch (error) {
    logger.error(`❌ Lỗi commitFundAccount cho ${accountId}:`, error);
    throw error;
  }
}

module.exports = {
  commitFundAccountLogic,
};
